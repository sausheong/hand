...
class ResultTests(unittest.TestCase):
    ...
    def test_cli_status_fingerprints_and_existing_report_preservation(self):
        for task in self.manifest['tasks']:
            for fixture in task['fixtures']:
                destination = self.root / fixture['source']
                destination.parent.mkdir(parents=True, exist_ok=True)
                destination.write_bytes((fixtures.ROOT / fixture['source']).read_bytes())
        manifest, results = self.root/'manifest.json', self.root/'results.json'
        manifest.write_text(json.dumps(self.manifest))
        results.write_text(json.dumps(self.results))
        script = Path(__file__).with_name('evaluation_results.py')
        output = self.root/'report.json'
        def run(path):
            return subprocess.run([sys.executable, str(script), str(manifest), str(results), '--out', str(path)],
                                  capture_output=True, text=True, timeout=10)
        completed = run(output)
        self.assertEqual(completed.returncode, 0, completed.stderr)
        original = output.read_bytes()
        report = json.loads(original)
        self.assertEqual(report['status'], 'records_complete_unreviewed')
        self.assertEqual(report['inputs'][str(results.resolve())], hashlib.sha256(results.read_bytes()).hexdigest())
        repeated = run(output)
        self.assertEqual(repeated.returncode, 2)
        self.assertEqual(json.loads(repeated.stderr)['status'], 'invalid')
        self.assertEqual(output.read_bytes(), original)
        self.results['runs'].pop()
        results.write_text(json.dumps(self.results))
        incomplete = run(self.root/'incomplete.json')
        ...
        self.results['manifest_sha256'] = '0'*64
        results.write_text(json.dumps(self.results))
        invalid_output = self.root/'invalid.json'
        invalid = run(invalid_output)
