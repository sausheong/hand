#!/usr/bin/env python3
"""Verify actual CLI per-run deadline cancellation, persistence and explicit renewal."""
import argparse
import hashlib
...
def main():
    ...
    try:
        ...
        client.call('time-start', 'budget.run.time.decide', {'id': 'timed-run', 'seconds': 2, 'confirmed': True})
        client.call('select-run', 'budget.run.select', {'id': 'timed-run', 'confirmed': True})
        configured = client.call('configured', 'budget.run')['time']
        ...
        assert expired['status'] == 'budget_exhausted' and expired['reason'] == 'run_time_budget'
        ...
        state = client.call('expired-state', 'budget.run')['time']
        ...
        restored = client.call('restored', 'budget.run')['time']
        assert restored['expired'] and restored['deadline'] == state['deadline']
        client.call('time-start', 'budget.run.time.decide', {'id': 'timed-run', 'seconds': 2, 'confirmed': True})
        assert client.call('after-replay', 'budget.run')['time']['deadline'] == state['deadline']
        client.call('still-expired', 'prompt', {'text': 'Do not renew the deadline implicitly.'})
        assert client.terminal('still-expired')['result']['payload']['reason'] == 'run_time_budget'
        assert len(requests) == 1, 'expired restart dispatched provider'
        client.call('renew', 'budget.run.time.decide', {'id': 'timed-run', 'seconds': 60, 'confirmed': True})
        renewed = client.call('renewed', 'budget.run')['time']
