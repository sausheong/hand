...
def validate(manifest, root):
    ...
    def money(value): return type(value) in (float, int) and math.isfinite(value) and value >= 0
    def relative(value):
    ...
    if manifest['planned_total_usd'] < planned_runs*budget['max_cost_usd']:gaps.append('planned total does not cover all per-run caps')
