	...
	for _, kind := range []string{"cli_jsonl", "subprocess_sdk", "embedded_sdk"} {
		...
		if kind == "cli_jsonl" {
			...
			if kind == "subprocess_sdk" {
				c, e = sdk.StartProcess(ctx, sdk.ProcessOptions{Binary: binary, Arguments: []string{"--rpc", "--model", "local/fixture", "--base-url", server.URL + "/v1"}, Directory: home, Environment: []string{"HOME=" + home, "PATH=/usr/bin:/bin"}})
			} else {
