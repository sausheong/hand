...
def validate(manifest, root):
    ...
    def relative(value, directory=False):
        if not string(value) or '\\' in value or '\x00' in value: return False
        path=PurePosixPath(value)
        return (not path.is_absolute() and '..' not in path.parts
                and path.as_posix()==value and (directory or value!='.')
                and all(part.lower()!='.git' for part in path.parts))
    ...
    for task in tasks:
        ...
        require(relative(task.get('working_directory'), directory=True), 'safe working directory required')
        ...
        for fixture in fixtures:
            ...
            require(not any(destination in PurePosixPath(other).parents or PurePosixPath(other) in destination.parents for other in destinations), 'overlapping fixture destinations')
            destinations.add(fixture['destination'])
            path=root.resolve()
            for part in PurePosixPath(fixture['source']).parts:
                path=path/part
                require(not path.is_symlink(), 'fixture cannot use symlink components')
