...
// Durable outcome flags remain authoritative when a provider supplied no text.
// Keep the transcript preview and full output viewer on the same projection.
func replayToolError(data session.ToolResultData) string {
	if data.Error != "" {
		return data.Error
	}
	if data.Aborted {
...
func outputFromEntry(entry session.SessionEntry) (ToolOutput, bool) {
	...
	if decodeReplayData(entry.Data, &data) != nil {
		return ToolOutput{}, false
	}
	return ToolOutput{Output: data.Output, Error: replayToolError(data), ToolID: data.ToolCallID}, true
}
